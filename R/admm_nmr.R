...
admm_nmr_fit <- function(max_iter,ep_abs,ep_rel, A,B){
  p<-dim(A)[1]; q<-dim(A)[2]; n<-dim(A)[3];
  # step 1
  ...
  x<-x_new; Y<-Y_new; Z<-Z_new; A_x<-A_x_new;
...
admm_nmr_clf <- function(A,B,label){
  x <- ADMM_NMR_fit(A,B)
  A_x <- coef_img(A,x)
...
admm_nmr_clf_top <- function(A,B,label,top_num){
  x <- ADMM_NMR_fit(A,B)
  A_x <- coef_img(A,x)
  # A_x_i 구하기 + error 구하기
  label_unique <- unique(label)
